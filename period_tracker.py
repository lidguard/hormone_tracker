...
header = ["DATE", "USERNAME", "SYMPTOM", "SEVERITY"]
data = [["default date", "default username", "default symptom", "default severity"]]
...
def grade():
        ...
        #write the confirmed symptom and severity to csv, inc date and username
        entry = pd.DataFrame({"DATE": [today],
                              "USERNAME": [f"{username}"],
                              "SYMPTOM": [f"{symptom}"],
                              "SEVERITY": [f"{severity}"]})
        entry.to_csv("symptoms.csv", mode="a", index=False, header=False)
